...
/// Pushes a computed `search_rank` column onto the SELECT list using pg_trgm similarity.
...
fn push_search_rank_select(qb: &mut QueryBuilder<sqlx::Postgres>, search_query: &str) {
    qb.push(", (similarity(input::text,");
    qb.push_bind(search_query.to_string());
    qb.push(") + similarity(output::text,");
    qb.push_bind(search_query.to_string());
...
fn add_common_where_clauses(
    ...
    if let Some(query) = search_query {
        ...
        // Accelerated by pg_trgm GIN indexes on input::text and output::text.
