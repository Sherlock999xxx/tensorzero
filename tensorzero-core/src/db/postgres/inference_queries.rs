...
enum OrderByTermResolved {
    ...
    Column(String),
    /// A similarity expression for search relevance ordering.
    /// The search query string is captured at build time (after validation).
    ...
    fn push_order_by(&self, qb: &mut QueryBuilder<sqlx::Postgres>) {
        ...
        for (i, (term, direction)) in self.terms.iter().enumerate() {
            ...
            match term {
                ...
                OrderByTermResolved::SearchRelevance(query) => {
                    qb.push("(similarity(io.input::text,");
                    qb.push_bind(query.clone());
                    qb.push(") + similarity(io.output::text,");
                    qb.push_bind(query.clone());
        ...
        for o in order_by {
            let term = match &o.term {
                ...
                OrderByTerm::SearchRelevance => {
                    let search_query = params.search_query_experimental.ok_or_else(|| {
                        Error::new(ErrorDetails::InvalidRequest {
                            message: "ORDER BY search_relevance requires search_query_experimental to be provided".to_string(),
                        })
                    })?;
                    OrderByTermResolved::SearchRelevance(search_query.to_string())
                }
            };
...
fn build_chat_inferences_query(
    ...
    query_builder.push(" WHERE io.id IS NOT NULL");
    ...
    // Apply search query filter (accelerated by pg_trgm GIN indexes)
...
fn build_inferences_union_query(
    ...
    let mut search_query_for_rank: Option<&str> = None;
    ...
    if let Some(order_by) = params.order_by {
        for o in order_by {
            let column = match &o.term {
                ...
                OrderByTerm::SearchRelevance => {
                    let sq = params.search_query_experimental.ok_or_else(|| {
                        Error::new(ErrorDetails::InvalidRequest {
                            message:
    ...
    // Add search_rank column if SearchRelevance ordering is requested
    if let Some(sq) = search_query_for_rank {
        push_similarity_search_rank(&mut query_builder, sq);
    }
...
/// Pushes a computed `search_rank` column using pg_trgm similarity onto the SELECT list.
fn push_similarity_search_rank(qb: &mut QueryBuilder<sqlx::Postgres>, search_query: &str) {
    qb.push(", (similarity(io.input::text,");
...
mod tests {
    ...
    fn test_build_chat_inferences_query_basic() {
        ...
        assert_query_equals(
            ...
            FROM tensorzero.chat_inferences i
            LEFT JOIN tensorzero.chat_inference_data io ON io.id = i.id AND io.created_at = i.created_at
            WHERE io.id IS NOT NULL AND i.function_name = $1
            ORDER BY i.id DESC
            LIMIT $2 OFFSET $3
            ",
        );
    ...
    fn test_build_chat_inferences_query_with_pagination_before() {
        ...
        assert_query_equals(
            ...
            FROM tensorzero.chat_inferences i
            LEFT JOIN tensorzero.chat_inference_data io ON io.id = i.id AND io.created_at = i.created_at
            WHERE io.id IS NOT NULL AND i.function_name = $1 AND i.id < $2
            ORDER BY i.id DESC
            LIMIT $3 OFFSET $4
            ",
        );
    ...
    fn test_build_inferences_union_query_basic() {
        ...
        assert_query_equals(
            ...
            SELECT * FROM (
                ...
                FROM tensorzero.chat_inferences i
                LEFT JOIN tensorzero.chat_inference_data io ON io.id = i.id AND io.created_at = i.created_at
                WHERE io.id IS NOT NULL ORDER BY id DESC LIMIT $1)
                UNION ALL
                (SELECT
                    'json'::text as inference_type,
                    i.id,
                ...
                FROM tensorzero.json_inferences i
                LEFT JOIN tensorzero.json_inference_data io ON io.id = i.id AND io.created_at = i.created_at
                WHERE io.id IS NOT NULL ORDER BY id DESC LIMIT $2)
            ) AS combined
