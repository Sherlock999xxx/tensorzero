...
///
/// Finds matching episode_ids first via `filtered_episodes`, applies pagination
/// and limit in `top_episodes`, then aggregates only those episodes.
fn build_filtered_episode_query(
    ...
    // CTE 1: Find episode_ids matching the filter criteria
    qb.push(
        r"
    ...
    // CTE 2: Apply pagination and limit to the filtered episode_ids
    // so the expensive aggregation only runs on N episodes.
    qb.push(
        r"
        ),
        top_episodes AS (
            SELECT episode_id FROM filtered_episodes",
    );
    if let Some(before_id) = before {
        qb.push(" WHERE episode_id < ");
        qb.push_bind(before_id);
    ...
    // CTE 3: Get all inferences for the top episodes (for full aggregation)
    qb.push(format!(
        r"
        ),
        all_inferences AS (
...
async fn query_episode_table_bounds_impl(pool: &PgPool) -> Result<TableBoundsWithCount, Error> {
    // Bounds: find the episode_id of the oldest/newest inference using PK order (id).
    // Since id is UUID v7 (time-ordered), ORDER BY id uses the PK index (id, created_at)
    // which is better cached than the secondary episode_id index.
    // The episode_id of the first/last inference is approximately the min/max episode_id.
    // Count: pg_class.reltuples for an approximate total (updated by autovacuum/analyze).
    sqlx::query_as(
        r"
        SELECT
            (SELECT episode_id FROM (
                (SELECT episode_id FROM tensorzero.chat_inferences ORDER BY id ASC LIMIT 1)
                UNION ALL
                (SELECT episode_id FROM tensorzero.json_inferences ORDER BY id ASC LIMIT 1)
            ) t ORDER BY episode_id ASC LIMIT 1) as first_id,
            (SELECT episode_id FROM (
                (SELECT episode_id FROM tensorzero.chat_inferences ORDER BY id DESC LIMIT 1)
                UNION ALL
                (SELECT episode_id FROM tensorzero.json_inferences ORDER BY id DESC LIMIT 1)
...
mod tests {
    ...
    fn test_build_query_episode_table_with_function_name_and_pagination() {
        ...
        assert_query_equals(
            ...
            top_episodes AS (
                SELECT episode_id FROM filtered_episodes
                WHERE episode_id < $3
                ORDER BY episode_id DESC LIMIT $4
            ),
            all_inferences AS (
                SELECT episode_id, id, created_at FROM tensorzero.chat_inferences
                WHERE episode_id IN (SELECT episode_id FROM top_episodes)
    ...
    fn test_build_query_episode_table_with_tag_filter() {
        ...
        assert_query_equals(
            ...
            WITH filtered_episodes AS (
                ...
                UNION
                SELECT DISTINCT i.episode_id FROM tensorzero.json_inferences i
                WHERE 1=1 AND i.tags @> jsonb_build_object($3, $4)
            ),
            top_episodes AS (
                SELECT episode_id FROM filtered_episodes
                ORDER BY episode_id DESC LIMIT $5
    ...
    fn test_build_query_episode_table_with_function_name_and_filter() {
        ...
        assert_query_equals(
            ...
            WITH filtered_episodes AS (
                SELECT DISTINCT i.episode_id FROM tensorzero.chat_inferences i
                WHERE 1=1 AND i.function_name = $1 AND i.tags @> jsonb_build_object($2, $3)
                UNION
                SELECT DISTINCT i.episode_id FROM tensorzero.json_inferences i
                WHERE 1=1 AND i.function_name = $4 AND i.tags @> jsonb_build_object($5, $6)
            ),
            top_episodes AS (
                SELECT episode_id FROM filtered_episodes
