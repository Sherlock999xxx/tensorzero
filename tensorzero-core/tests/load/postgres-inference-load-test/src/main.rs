...
async fn main() -> Result<()> {
    ...
    let db_metrics = verify_db_parity(
        &postgres_url,
        &run_id,
        run_stats
            .successful_requests
    ...
    let db_correctness = if db_metrics.verified_within_timeout {
        CheckResult {
            passed: true,
            reason: format!(
                "DB parity verified: inference_rows={} model_inference_rows={} expected_inference_rows={}",
                db_metrics.inference_rows,
                db_metrics.model_inference_rows,
                db_metrics.expected_inference_rows
            ),
        }
    } else {
        CheckResult {
            passed: false,
            reason: format!(
                "DB parity failed: inference_rows={} model_inference_rows={} expected_inference_rows={}",
                db_metrics.inference_rows,
                db_metrics.model_inference_rows,
                db_metrics.expected_inference_rows
            ),
...
async fn verify_db_parity(
    ...
    loop {
        let inference_rows_match = i64::try_from(expected_inference_rows)
            .ok()
            .map(|expected| inference_rows == expected)
            .unwrap_or(false);
        let model_rows_match = model_inference_rows >= inference_rows;

        if inference_rows_match && model_rows_match {
            return Ok(DbMetrics {
...
fn print_summary(summary: &BenchmarkSummary) {
    ...
    println!(
        "  db_rows: inference={} model_inference={} expected_inference={}",
        summary.db_metrics.inference_rows,
        summary.db_metrics.model_inference_rows,
        summary.db_metrics.expected_inference_rows
    );
    println!();
    println!(
        "  steady_state: {} ({})",
